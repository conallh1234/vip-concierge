...
export default function LoginForm() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (userData.user.role === 'ADMIN' || userData.user.role === 'USER') {
                router.push('/dashboard');
            } else if (userData.user.role === 'CUSTOMER') {
                router.push('/client');
            } else {
                console.warn('⚠️ Unrecognized role, defaulting to client.');
                router.push('/client');
            }
        ...
        <form onSubmit={handleLogin} className="max-w-md mx-auto p-8 bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-6">
                ...
                <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full py-3 px-4 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center"
                >
                    {isLoading ? (
                        <>
